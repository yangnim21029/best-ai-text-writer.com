...
export const runAnalysisPipelineService = async (
  ...
  const productTask = async () => {
    ...
    if (parsedProductBrief && parsedProductBrief.productName) {
      ...
      const mapRes = await generateProblemProductMapping(
        parsedProductBrief,
  ...
  const keywordTask = async () => {
    ...
    const finalLimit = Math.max(
      appStore.minKeywords,
      Math.min(appStore.maxKeywords, calculatedLimit)
    );
    ...
    if (keywordPlanCandidates.length > 0 && !isStopped()) {
      ...
      try {
        ...
        const planRes = await extractSemanticKeywordsAnalysis(
          fullConfig.referenceContent,
          keywords,
          fullConfig.targetAudience
        );
  ...
  const visualTask = async () => {
    ...
    try {
      const styleRes = await analyzeVisualStyle(
        initialImages,
        fullConfig.websiteType || 'Modern Business'
      );
  ...
  const regionalTask = async () => {
    ...
    try {
      const regionRes = await analyzeRegionalTerms(
        fullConfig.referenceContent,
        fullConfig.targetAudience
      );
