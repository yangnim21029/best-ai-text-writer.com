...
export const validateAndAdaptForRegion = async (
  ...
  console.log(
    `[RegionGrounding] Detected ${detectResult.entities.length} foreign entities for ${regionName}`
  );
  ...
  console.log(
    `[RegionGrounding] Found ${groundingResult.mappings.length} ${regionName} equivalents`
  );
