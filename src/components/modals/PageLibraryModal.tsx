...
import { Search, FileText, LayoutTemplate, Briefcase, Trash2, CheckCircle2, MoreVertical, Plus, X, Globe, Info, Clock, ExternalLink } from 'lucide-react';
            ...
            <div className="relative w-full max-w-6xl h-full max-h-[850px] bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-indigo-100 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                ...
                <div className="px-10 py-8 border-b border-indigo-50 flex items-center justify-between bg-indigo-50/20">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-1">Manage your article topics, source materials, and outlines</p>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {filteredPages.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-6">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-sm text-slate-400 mt-1">Save your current article settings as a profile for easy reuse.</p>
                            </div>
                            <button
                                onClick={() => setIsCreating(true)}
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-start justify-between mb-5">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-2 transition-colors ${isActive ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50 border-slate-50 group-hover:bg-indigo-50 group-hover:border-indigo-100'}`}>
                                                <FileText className={`w-7 h-7 ${isActive ? 'text-indigo-600' : 'text-slate-300 group-hover:text-indigo-600'}`} />
                                            </div>
                                        ...
                                        <div className="mt-4 bg-slate-50/50 rounded-2xl p-3 border border-slate-100 group-hover:bg-indigo-50/30 transition-colors">
                                            ...
                                            <p className="text-[11px] text-slate-500 line-clamp-2 leading-relaxed">
                                                {page.referenceContent ? page.referenceContent.substring(0, 100) : 'No content provided'}
                                            </p>
                                        </div>
                                    </div>
                                );
