...
import { Search, ShoppingBag, Trash2, CheckCircle2, Plus, X, FileText, Sparkles, Copy, ExternalLink, MoreVertical } from 'lucide-react';
            ...
            <div className="relative w-full max-w-6xl h-full max-h-[850px] bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-emerald-100 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                ...
                <div className="px-10 py-8 border-b border-emerald-50 flex items-center justify-between bg-emerald-50/20">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-1">Store and pick the best product descriptions for your articles</p>
                    </div>
                    <button
                        onClick={onClose}
                                ...
                                return (
                                    ...
                                    >
                                        <div className="min-w-0 flex-1">
                                            <p className={`text-xs font-bold truncate ${isActive ? 'text-white' : 'text-slate-800'}`}>{profile.name}</p>
                                            <p className={`text-[10px] truncate ${isActive ? 'text-emerald-100' : 'text-slate-400'}`}>
                                                {profile.productRawText ? profile.productRawText.substring(0, 40) + '...' : 'No details'}
                                            </p>
                                        </div>
                    ...
                    <div className="flex-1 bg-white flex flex-col">
                        {isCreating || activeProfileId ? (
                                ...
                                <div className="max-w-3xl w-full mx-auto space-y-8">
                                    <div className="space-y-4">
                                        ...
                                        <input
                                            ...
                                            value={isCreating ? newName : profiles.find(p => p.id === activeProfileId)?.name}
                                            readOnly={!isCreating}
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <textarea
                                            ...
                                            value={isCreating ? newContent : (editingContent !== null ? editingContent : profiles.find(p => p.id === activeProfileId)?.productRawText)}
                                            onChange={(e) => isCreating ? setNewContent(e.target.value) : setEditingContent(e.target.value)}
                                    ...
                                    <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <button
                                                onClick={() => {
                                                    const text = isCreating ? newContent : (editingContent || profiles.find(p => p.id === activeProfileId)?.productRawText);
                                                    if (text) {
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-slate-300 p-10 text-center animate-in fade-in duration-500">
                                ...
                                <p className="text-sm text-slate-500 max-w-xs mx-auto">Click a service on the left to view or edit its details. AI will use this info to replace competitor names.</p>
                            </div>
                        )}
                    </div>
                </div>
