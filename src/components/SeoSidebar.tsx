...
import {
    BrainCircuit, Layers, Target, ShieldCheck, Database,
    Zap, Hash, BarChart2, FileSearch, ArrowRight, Gem,
    Square, Languages, Copy, Check, Globe, LayoutTemplate,
    RotateCcw, Search, Loader2, ShoppingBag, ListChecks,
    Quote, MessageSquare, AlertCircle, Info, Bookmark, FileText
} from 'lucide-react';
import {
    GenerationStatus, FrequentWordsPlacementAnalysis, ReferenceAnalysis,
...
export const SeoSidebar: React.FC<SeoSidebarProps> = ({
    ...
    const detectedBrands = useMemo(() => {
        return (currentRefAnalysis?.competitorBrands || []).filter(brand =>
    ...
    const detectedProducts = useMemo(() => {
        return (currentRefAnalysis?.competitorProducts || []).filter(product =>
    ...
    const difficultyBadge = (value?: string) => {
        ...
        return (
            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border ${variant.bg} ${variant.text} ${variant.border}`}>
                {variant.label}
            </span>
        );
    };
            ...
            <div className="space-y-2">
                ...
                {section.logicalFlow && (
                    <div className="flex gap-2 items-start mt-1">
                        ...
                        <p className="text-[10px] text-gray-500 italic leading-snug">Arc: {section.logicalFlow}</p>
                    </div>
                )}
                ...
                {section.solutionAngles && section.solutionAngles.length > 0 && (
                    <div className="space-y-1 bg-blue-50/30 p-2 rounded-lg border border-blue-100/50">
                        <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Solution Angles</p>
                        {section.solutionAngles.map((angle, aIdx) => (
                            <div key={aIdx} className="text-[10px] text-blue-700 flex items-center gap-1.5 font-bold">
                ...
                {section.narrativePlan && section.narrativePlan.length > 0 && (
                    <div className="space-y-1">
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Narrative Action</p>
                ...
                {section.uspNotes && section.uspNotes.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                        ...
                        {section.uspNotes.map((usp, uIdx) => (
                            <span key={uIdx} className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-100 rounded text-[9px] font-black uppercase tracking-tighter">
                                USP: {usp}
                            </span>
                        ))}
                    </div>
                ...
                {section.sentenceStartFeatures && section.sentenceStartFeatures.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                        {section.sentenceStartFeatures.map((feat, idx) => (
                            <span key={idx} className="text-[8px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200 font-bold tracking-tighter">Start: {feat}</span>
                        ))}
                    </div>
                )}
                ...
                {section.sentenceEndFeatures && section.sentenceEndFeatures.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                        {section.sentenceEndFeatures.map((feat, idx) => (
                            <span key={idx} className="text-[8px] bg-pink-50 text-pink-500 px-1.5 py-0.5 rounded border border-pink-100 font-bold tracking-tighter">End: {feat}</span>
                        ))}
                    </div>
                )}
                ...
                {!currentRefAnalysis && !currentKeywords.length && status !== 'analyzing' && !isDisplayingDocument ? (
                    ...
                    <>
                            ...
                            <div className="p-4 bg-gradient-to-br from-blue-600 via-blue-600 to-blue-700 rounded-2xl shadow-xl shadow-blue-500/20 border border-blue-400/30 overflow-hidden relative group">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-2.5 mb-2">
                                        ...
                                        <span className="text-[11px] font-black text-white uppercase tracking-[0.1em]">Processing Phase: Intelligence Extraction</span>
                                    </div>
                                    <p className="text-[10px] text-blue-50 font-medium leading-relaxed max-w-[90%]">
                                        Gemini 2.0 is mapping semantic clusters, regional linguistic nuances, and competitive positioning...
                                    </p>
                                </div>
                            </div>
                        )}
                                                        ...
                                                        {map.relevanceKeywords?.length > 0 && (
                                                            <div className="flex flex-wrap gap-1 mt-2">
                                                                {map.relevanceKeywords.slice(0, 3).map((k, ki) => (
                                                                    <span key={ki} className="text-[8px] text-slate-400 bg-white px-1.5 py-0.5 rounded border border-slate-100">#{k}</span>
                                                                ))}
                                                            </div>
                                                        )}
                                                    </div>
                        ...
                        {(currentRefAnalysis?.humanWritingVoice || currentRefAnalysis?.toneSensation || currentVisualStyle) && (
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-4 group hover:shadow-md transition-all duration-300">
                                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                    <MessageSquare className="w-3.5 h-3.5 text-purple-500" /> Tactical Voice Architecture
                                </h3>
                                <div className="space-y-4">
                                    {currentRefAnalysis?.toneSensation && (
                                        <div className="flex gap-3 p-3 bg-purple-50/40 rounded-2xl border border-purple-100/50">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[11px] text-purple-900 font-bold leading-relaxed">{currentRefAnalysis.toneSensation}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {currentVisualStyle && (
                                        <div className="p-4 bg-amber-50/20 rounded-2xl border border-amber-100/40 group-hover:bg-white transition-colors border-dashed">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-[10px] font-black text-amber-700 uppercase tracking-widest">Visual Style Guide</span>
                                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {currentKeywords.length > 15 && (
                                    <div className="py-2 text-center">
                                        <span className="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em] font-mono">+ {currentKeywords.length - 15} Additional Nodes Hidden</span>
                                    </div>
                                )}
                            </div>
                        )}
                                            ...
                                            <div className="p-4 bg-rose-50/50 border border-rose-100 rounded-2xl shadow-sm relative overflow-hidden group/alert">
                                                ...
                                                <div className="flex flex-wrap gap-1.5 mb-4">
                                                    {detectedBrands.map((b, i) => (
                                                        <span key={`b-${i}`} className="bg-white px-2.5 py-1 rounded-xl border border-rose-200 text-rose-600 text-[10px] font-black shadow-sm shrink-0">
                                                            -{b}
                                                        </span>
                                                    ))}
                                                    {detectedProducts.map((p, i) => (
                                                        <span key={`p-${i}`} className="bg-white px-2.5 py-1 rounded-xl border border-orange-200 text-orange-600 text-[10px] font-black shadow-sm shrink-0">
                                                            -{p}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="space-y-2.5">
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                {currentRefAnalysis.regionalReplacements.map((item, i) => (
                                                    <div key={i} className="p-3 bg-white border border-gray-100 rounded-2xl flex flex-col gap-2 hover:border-amber-200 transition-all">
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                <span className="text-[11px] text-gray-400 line-through font-medium">{item.original}</span>
                                                        ...
                                                        {item.reason && (
                                                            <p className="text-[9px] text-gray-400 leading-normal italic px-1">{item.reason}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        {currentRefAnalysis.keyInformationPoints && currentRefAnalysis.keyInformationPoints.length > 0 && (
                                            <div className="space-y-3">
                                                ...
                                                {currentRefAnalysis.keyInformationPoints.map((point, idx) => (
                                                    <div key={idx} className="flex gap-3 text-[11px] text-slate-300 leading-relaxed font-medium">
