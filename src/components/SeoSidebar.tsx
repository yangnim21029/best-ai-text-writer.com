...
export const SeoSidebar: React.FC<SeoSidebarProps> = ({
  ...
  const currentProductMapping = (isDisplayingDocument ? activeDoc?.productMapping : liveMapping) || [];
  const currentProductBrief = isDisplayingDocument ? activeDoc?.productBrief : liveBrief;
  const currentAudience = (isDisplayingDocument ? activeDoc?.targetAudience : liveAudience) || 'zh-TW';
  const currentLangInst = (isDisplayingDocument ? activeDoc?.languageInstruction : liveLangInst) || '';
  const currentVisualStyle = isDisplayingDocument ? activeDoc?.visualStyle : analysisStore.visualStyle;
  ...
  const handleSynthesize = async () => {
    ...
    try {
      ...
      analysisStore.setSelectedDocumentIds([]);
      ...
      <div className="p-4 border-b border-gray-200 bg-white sticky top-0 z-20 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-xl bg-blue-600 text-white ${!isDisplayingDocument && 'shadow-lg shadow-blue-500/20'}`}>
            ...
            <div>
              <h2 className="text-[12px] font-black text-gray-900 tracking-tight leading-none mb-1">
                {isDisplayingDocument ? 'RECORDED DOCUMENT' : '語氣與大綱分析'}
              </h2>
          ...
          <div className="flex flex-col gap-3">
            ...
            {status === 'analyzing' && (
              <button onClick={onStop} className="px-3 py-2 text-[10px] font-extrabold text-red-600 bg-red-50 hover:bg-red-100 border border-red-100 rounded-xl transition-all flex items-center gap-1.5">
        ...
        {!currentRefAnalysis && !(currentKeywords?.length ?? 0) && status !== 'analyzing' && !isDisplayingDocument ? (
          ...
          <>
            {status === 'analyzing' && !isDisplayingDocument && (
              <div className="p-4 bg-gradient-to-br from-blue-600 via-blue-600 to-blue-700 rounded-2xl shadow-xl border border-blue-400/30 overflow-hidden relative group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2.5 mb-2">
                    ...
                    <span className="text-[11px] font-black text-white uppercase tracking-[0.1em]">Intelligence Extraction</span>
                  </div>
                  <p className="text-[10px] text-blue-50 font-medium leading-relaxed max-w-[90%]">
                    Gemini 2.0 is mapping semantic clusters, regional nuances, and competitive positioning...
                  </p>
                </div>
              </div>
            )}
                    ...
                    <button onClick={() => setShowLangDetails(!showLangDetails)} className="text-[9px] font-black text-gray-400 hover:text-blue-600 px-2 py-1 bg-gray-50 rounded-lg hover:bg-white transition-all uppercase tracking-tight">
                      {showLangDetails ? 'Collapse' : 'Expand'}
                    </button>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 transition-colors">
                    <span className="text-[10px] font-black text-gray-400 uppercase">Target Market</span>
                    <span className="text-[13px] font-black text-slate-800">
                      {currentAudience === 'zh-TW' ? '🇹🇼 Taiwan / 繁中' : currentAudience === 'zh-HK' ? '🇭🇰 HK / 繁中' : '🇲🇾 MY / 簡中'}
                    </span>
                  </div>
                  <div className={`relative px-4 py-3 bg-indigo-50/20 rounded-2xl border border-blue-100/40 transition-all duration-500 ${showLangDetails ? 'max-h-[800px]' : 'max-h-[100px] shadow-inner'}`}>
                    ...
                    {!showLangDetails && currentLangInst && <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-indigo-50/40 to-transparent pointer-events-none" />}
                  </div>
                </div>
              </div>
            )}
            ...
            {(currentProductBrief || currentProductMapping.length > 0) && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 group hover:shadow-md transition-all duration-300">
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  <ShoppingBag className="w-3.5 h-3.5 text-emerald-500" /> Value Proposition Mapping
                </h3>
                <div className="space-y-4">
                  {currentProductBrief && (
                    <div className="p-4 bg-emerald-50/30 rounded-2xl border border-emerald-100/50 relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-2 opacity-5"><Gem className="w-12 h-12 text-emerald-600" /></div>
                      <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Target className="w-3 h-3" /> Core Solution</div>
                      <div className="text-[12px] font-black text-gray-800 mb-1">{currentProductBrief.brandName} • {currentProductBrief.productName}</div>
                      <p className="text-[10px] text-emerald-700/80 leading-relaxed font-medium mt-2 p-2 bg-emerald-100/30 rounded-lg">{currentProductBrief.usp}</p>
                    </div>
                  )}
                  {currentProductMapping.length > 0 && (
                    <div className="space-y-2.5">
                      ...
                      {currentProductMapping.map((map, i) => (
                        <div key={i} className="flex gap-3 p-3 bg-slate-50/50 rounded-2xl border border-slate-100 hover:bg-white hover:border-blue-100 transition-all">
                          <div className="w-2 h-2 rounded-full bg-blue-500 mt-2 flex-shrink-0" />
                          <div className="min-w-0">
                            <div className="text-[11px] font-bold text-gray-700 leading-snug mb-1.5">"{map.painPoint}"</div>
                            <div className="flex items-center gap-1.5 px-2 py-1 bg-white border border-blue-100 rounded-lg w-fit">
                              <ArrowRight className="w-3 h-3 text-blue-400" /><span className="text-[10px] font-black text-blue-600 truncate uppercase tracking-tighter">{map.productFeature}</span>
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            {(currentRefAnalysis?.humanWritingVoice || currentRefAnalysis?.toneSensation || currentVisualStyle) && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-4 group hover:shadow-md transition-all duration-300">
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  <MessageSquare className="w-3.5 h-3.5 text-purple-500" /> 品牌語氣大綱
                </h3>
                ...
                <div className="space-y-4">
                  {currentRefAnalysis?.toneSensation && (
                    <div className="flex gap-3 p-3 bg-purple-50/40 rounded-2xl border border-purple-100/50">
                      <div className="w-6 h-6 rounded-lg bg-white border border-purple-100 flex items-center justify-center shrink-0 shadow-sm"><MessageSquare className="w-3 h-3 text-purple-400" /></div>
                      <div>
                        <p className="text-[9px] font-black text-purple-400 uppercase tracking-widest mb-0.5">Vibe / Sensation</p>
                        <p className="text-[11px] text-purple-900 font-bold">{currentRefAnalysis.toneSensation}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {currentVisualStyle && (
                    <div className="p-4 bg-amber-50/20 rounded-2xl border border-amber-100/40 transition-colors border-dashed">
                      <div className="flex items-center gap-2 mb-2"><LayoutTemplate className="w-3.5 h-3.5 text-amber-500" /><span className="text-[10px] font-black text-amber-700 uppercase tracking-widest">Visual Style Guide</span></div>
                      <p className="text-[11px] text-amber-900 leading-[1.6] font-medium italic">{currentVisualStyle}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 group hover:shadow-md transition-all duration-300">
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {currentAuthAnalysis.relevantTerms?.slice(0, 18).map((term, i) => (
                    <div key={i} className="px-2.5 py-1 bg-indigo-50/50 text-blue-700 border border-blue-100 rounded-xl text-[10px] font-black">#{term}</div>
                  ))}
                </div>
                <div className="space-y-3">
                  {currentAuthAnalysis.combinations?.slice(0, 3).map((c, i) => (
                    <div key={i} className="p-3 bg-slate-50 rounded-2xl border border-slate-100 text-[11px] text-slate-700 italic">
                      <span className="text-indigo-300 font-black mr-2">0{i + 1}</span>{c}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {currentRefAnalysis && (totalDetectedCount > 0 || (currentRefAnalysis.regionalReplacements && currentRefAnalysis.regionalReplacements.length > 0)) && (
              <div className="bg-white rounded-2xl border border-rose-100 shadow-sm p-4 group hover:shadow-md transition-all duration-300">
                ...
                <div className="space-y-4">
                  {totalDetectedCount > 0 && (
                    <div className="p-4 bg-rose-50/50 border border-rose-100 rounded-2xl relative overflow-hidden">
                      <p className="text-[11px] font-black text-rose-700 mb-3">Blocked Competition ({totalDetectedCount})</p>
                  ...
                  {currentRefAnalysis.regionalReplacements && currentRefAnalysis.regionalReplacements.length > 0 && (
                    <div className="space-y-2.5">
                      <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest px-1">Linguistic Overrides</p>
                          ...
                          <div key={i} className="p-3 bg-white border border-gray-100 rounded-2xl flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-[11px] font-black text-emerald-600">{item.replacement || '(Nullify)'}</span>
                              </div>
                            </div>
                            {item.reason && <p className="text-[9px] text-gray-400 italic px-1">{item.reason}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {currentRefAnalysis?.structure && (
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2"><Layers className="w-3.5 h-3.5 text-blue-500" /> 文章大綱</h3>
                  <span className="text-[10px] font-black text-slate-300 font-mono">{currentRefAnalysis.structure.length} Sections</span>
                </div>
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-4 relative group hover:shadow-md transition-shadow">
                  {currentRefAnalysis.h1Title && (
                    <div className="mb-4 bg-indigo-50/50 p-4 rounded-2xl border border-blue-100/50">
                      <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1.5 flex items-center gap-2"><FileText className="w-3 h-3" /> Extracted H1 Title</p>
                      <p className="text-[13px] font-black text-blue-900">{currentRefAnalysis.h1Title}</p>
                    </div>
                  )}
                  {currentRefAnalysis.introText && (
                    <div className="mb-6 pb-6 border-b border-indigo-50 px-2">
                      <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Info className="w-3 h-3" /> Context Abstract</p>
                      <p className="text-[11px] text-gray-800 italic">"{currentRefAnalysis.introText}"</p>
                    </div>
                  )}
                  <div className="absolute left-[26px] top-8 bottom-8 w-px bg-slate-100 z-0" />
                  <div className="relative z-10">
                    {currentRefAnalysis.structure.map((section, idx) => <SectionItem key={idx} section={section} index={idx} />)}
                  </div>
                </div>
              </div>
            )}

            {currentRefAnalysis && (currentRefAnalysis.keyInformationPoints?.length || currentRefAnalysis.conversionPlan?.length || currentRefAnalysis.brandExclusivePoints?.length) && (
              <div className="bg-slate-800 rounded-2xl shadow-xl p-6 relative group">
            ...
            {currentRefAnalysis?.generalPlan && currentRefAnalysis.generalPlan.length > 0 && (
              <div className="p-4 bg-white border border-blue-100 rounded-2xl shadow-sm relative overflow-hidden group">
                ...
                <div className="space-y-3">
                  {currentRefAnalysis.generalPlan.map((point, idx) => <div key={idx} className="flex gap-3 text-[11px] text-gray-700 leading-relaxed"><span className="text-blue-400 font-black">•</span><span>{point}</span></div>)}
                </div>
                {currentRefAnalysis?.regionVoiceDetect && <div className="mt-4 pt-4 border-t border-indigo-50 flex items-center justify-between"><span className="text-[9px] font-black text-gray-400 uppercase">Linguistic Fingerprint</span><span className="text-[10px] font-black text-blue-600">{currentRefAnalysis.regionVoiceDetect}</span></div>}
              </div>
            )}
          </>
        )}
