...
export const SectionPlanModal: React.FC<SectionPlanModalProps> = ({
  ...
  const activeGeneralPlan =
    useLocalizedPlan && hasLocalizedPlan ? localizedGeneralPlan || [] : generalPlan;
  const activeConversionPlan =
    useLocalizedPlan && hasLocalizedPlan ? localizedConversionPlan || [] : conversionPlan;
  const activeHumanVoice =
    useLocalizedPlan && hasLocalizedPlan ? localizedHumanWritingVoice : humanWritingVoice;
      ...
      <div className="relative bg-white w-full max-w-6xl h-[85vh] rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/70 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl ${isSynthesis ? 'bg-indigo-600 text-white shadow-indigo-200' : 'bg-blue-600 text-white shadow-blue-200'} flex items-center justify-center shadow-lg relative`}
            >
                      ...
                      <div className="flex items-start justify-between gap-3">
                          ...
                          <label className="flex items-start gap-2 cursor-pointer select-none">
                            <input
                              ...
                              onChange={(e) =>
                                setDrafts((prev) =>
                                  prev.map((d, di) =>
                                    di === section.idx ? { ...d, selected: e.target.checked } : d
                                  )
                                )
                              }
                            />
                              ...
                              <>
                                <input
                                  ...
                                  onChange={(e) =>
                                    setDrafts((prev) =>
                                      prev.map((d, di) =>
                                        di === section.idx ? { ...d, title: e.target.value } : d
                                      )
                                    )
                                  }
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setDrafts((prev) =>
                                      prev.map((d, di) =>
                                        di === section.idx ? { ...d, coreFocus: e.target.value } : d
                                      )
                                    )
                                  }
                              ...
                              <>
                                ...
                                {section.data.logicalFlow ? (
                                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-medium text-blue-600 bg-blue-50/50 px-2 py-0.5 rounded-md border border-blue-100/50 w-fit">
                                    ...
                                    <span className="leading-none text-blue-700">
                                      邏輯鎖鍊：{section.data.logicalFlow}
                                    </span>
                                  </div>
                                ) : null}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {section.data.writingMode && (
                            <span
                              className={`px-2 py-0.5 text-[9px] font-black rounded-full border border-dashed ${section.data.writingMode === 'direct' ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-purple-50 text-purple-600 border-purple-200'}`}
                            >
                        ...
                        {isEditing ? (
                          <textarea
                            ...
                            onChange={(e) =>
                              setDrafts((prev) =>
                                prev.map((d, di) =>
                                  di === section.idx ? { ...d, narrativeText: e.target.value } : d
                                )
                              )
                            }
                          ...
                          {isEditing ? (
                            <textarea
                              ...
                              onChange={(e) =>
                                setDrafts((prev) =>
                                  prev.map((d, di) =>
                                    di === section.idx ? { ...d, keyFactsText: e.target.value } : d
                                  )
                                )
                              }
                          ...
                          {isEditing ? (
                            <textarea
                              ...
                              onChange={(e) =>
                                setDrafts((prev) =>
                                  prev.map((d, di) =>
                                    di === section.idx ? { ...d, uspText: e.target.value } : d
                                  )
                                )
                              }
                        ...
                        {isEditing ? (
                          <textarea
                            ...
                            onChange={(e) =>
                              setDrafts((prev) =>
                                prev.map((d, di) =>
                                  di === section.idx ? { ...d, subheadingsText: e.target.value } : d
                                )
                              )
                            }
                          ...
                          <div className="mt-2 space-y-2">
                            {section.data.subsections && section.data.subsections.length > 0 ? (
                              ...
                              section.data.subsections.map((sub, hi) => (
                                <div
                                  key={hi}
                                  className="bg-white border border-amber-100 rounded-lg p-2 space-y-1"
                                >
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-1.5 rounded">
                                      {hi + 1}
                                    </span>
                                    <span className="text-xs font-bold text-amber-800 break-words">
                                      {sub.title}
                                    </span>
                                  </div>
                            ...
                            {!section.data.subsections?.length &&
                              (!section.data.subheadings ||
