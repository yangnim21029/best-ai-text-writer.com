...
const ChecklistItem: React.FC<{
  ...
  ({ point, type, isChecked, isProcessing, onToggle, onRefine, useTiptap, isTiptapReady }) => {
      ...
      >
        <div className="flex items-start gap-2 pr-6 cursor-pointer" onClick={() => onToggle(point)}>
          {isChecked ? (
            ...
            <span className="w-4 h-4 text-orange-300 flex-shrink-0">☐</span>
          )}
          <div className="flex-1">
            {type === 'brand' && (
              <span className="text-[10px] font-bold text-purple-600 bg-purple-50 px-1 rounded mr-1">
                USP
              </span>
            )}
  ...
  return (
    <div className="w-72 bg-orange-50/50 border-l border-orange-100 p-4 overflow-y-auto custom-scrollbar animate-in slide-in-from-right duration-200">
      ...
      <div className="space-y-4">
        ...
        {keyPoints.length > 0 && (
          <div className="space-y-2">
            <h5 className="text-[10px] font-bold text-orange-400 uppercase tracking-wider">
              General Knowledge
            </h5>
            <div className="space-y-1">
