...
export const refineAllHeadings = async (
  ...
  const headingsInput = sectionHeadings.map((h, idx) =>
    cleanHeadingText(h || sectionsToGenerate[idx]?.title)
  );
  ...
  try {
    ...
    const refinedList = headingsInput.map((before, idx) => {
      const match =
        refineRes.data?.find((h: any) => cleanHeadingText(h.before) === before) ||
        refineRes.data?.[idx];
    ...
    const normalizedForUi = (refineRes.data || []).map((item: any, idx: number) => {
      ...
      const h2After = cleanHeadingText(
        item.h2_after || item.after || item.before || headingsInput[idx] || ''
      );
    ...
    appStore.addCost(refineRes.cost.totalCost, refineRes.usage.totalTokens);
