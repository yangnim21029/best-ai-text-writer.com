...
const NOISE_PATTERNS = {
  ads: [/advertisement/i, /ad placement/i, /sponsored/i, /贊助/, /廣告/],
  navigation: [
  ...
  ui: [/^continue reading below$/i, /scroll to continue/i, /read more/i, /繼續閱讀|閱讀更多/],
  social: [/^(facebook|instagram|twitter|line)/i, /點擊訂閱/],
  metadata: [/^published:?/i, /^author:?/i, /^tags?:/i, /發布時間|作者：|標籤：/],
};
...
function analyzeContent(content: string): ContentMetrics {
  ...
  if (noiseRatio > QUALITY_STANDARDS.maxNoiseRatio) {
    issues.push(
      `高噪音比例: ${(noiseRatio * 100).toFixed(1)}% (標準: <${QUALITY_STANDARDS.maxNoiseRatio * 100}%)`
    );
  }
  ...
  if (infoDensity < QUALITY_STANDARDS.minInfoDensity) {
    warnings.push(
      `資訊密度低: ${(infoDensity * 100).toFixed(1)}% (建議: >${QUALITY_STANDARDS.minInfoDensity * 100}%)`
    );
  }

  if (shortLineRatio > QUALITY_STANDARDS.maxShortLineRatio) {
    warnings.push(
      `過多短行: ${(shortLineRatio * 100).toFixed(1)}% (建議: <${QUALITY_STANDARDS.maxShortLineRatio * 100}%)`
    );
  }
...
async function runQualityTests(urls: string[]) {
  ...
  Object.entries(gradeCount).forEach(([grade, count]) => {
    const percentage =
      successful.length > 0 ? ((count / successful.length) * 100).toFixed(1) : '0.0';
  ...
  if (successful.length > 0) {
    ...
    const avgNoise =
      successful.reduce((sum, r) => sum + (r.metrics?.noiseRatio || 0), 0) / successful.length;
    const avgInfo =
      successful.reduce((sum, r) => sum + (r.metrics?.infoDensity || 0), 0) / successful.length;
    const avgReadability =
      successful.reduce((sum, r) => sum + (r.metrics?.readabilityScore || 0), 0) /
      successful.length;
  ...
  // Top Performers
  const topPerformers = [...successful].sort((a, b) => b.qualityScore - a.qualityScore).slice(0, 5);
  ...
  if (bottomPerformers.length > 0 || failed.length > 0) {
    const problemUrls = [...bottomPerformers.map((r) => r.url), ...failed.map((r) => r.url)];
